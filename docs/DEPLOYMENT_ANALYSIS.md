...
| `public_bucket_allows_listing` ×2 (`avatars`, `gallery`) | ✅ FIXED | Dropped `public_read_avatars` / `public_read_gallery` — public bucket URLs are CDN-served without RLS; the app never calls `storage.from(...).list()` (verified in source). **2026-08-10 update:** owner-scoped SELECT policies (`auth_select_own_avatars/resumes/gallery`) were added back via migration `add_owner_select_storage_upsert_fix` because `storage.upload(..., { upsert: true })` (avatar/resume) runs `INSERT ... ON CONFLICT` and fails with **42501** when the conflicting row is not visible to the role. Policies are `TO authenticated` only and scoped to `folder = auth.uid()`, so they do **not** re-enable public listing and lint 0025 stays clean |
