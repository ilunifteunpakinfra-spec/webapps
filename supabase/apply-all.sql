-- ============================================
-- ILUNI FTE WebApps - ONE-SHOT DEPLOYMENT SQL
-- Generated from reset.sql + schema.sql + storage-policies.sql
...
DROP TABLE IF EXISTS group_thread_replies CASCADE;
DROP TABLE IF EXISTS group_threads CASCADE;
...
-- Forum diskusi grup: thread + balasan (fitur komunitas, lihat 0008_group_forum.sql)
...
-- ============================================
-- 5.11B GROUP FORUM POLICIES (thread + balasan)
-- ============================================
-- Helper: apakah user saat ini anggota grup tsb? SECURITY DEFINER agar
-- policy dapat mengecek keanggotaan tanpa memicu RLS ulang (menghindari
-- infinite recursion, pola sama seperti is_group_admin).
...
-- ============================================
-- 9. CONTENT MODERATION (Phase 2)
-- ============================================
-- Mirrors supabase/migrations/0003_content_moderation.sql so the
-- canonical schema stays 1:1 with the live database.

-- 9.1 SOFT-HIDE STATUS COLUMNS
-- Option C (hybrid moderation): reversible soft-hide for job postings
-- and announcements via a `status` column ('active' | 'hidden').
